...
def read_raw_coverage_data(input_path):
    ...
    """
    chunks = []
    for chunk in pd.read_csv(str(input_path), sep='\t', compression='gzip',
                             chunksize=1_000_000):
        chunks.append(chunk)
...
def read_coverage_data_v2(path):
    ...
    after filtering out X and Y chromosomes."""
    df_cov = read_raw_coverage_data(str(path))
    df_cov = df_cov[~df_cov['chrom'].isin(['chrX', 'chrY'])]
...
def main(input_dir, output_dir, gene_config_path):
    ...
    if not os.path.exists(df_cov_v2_path):
    ...
    if not os.path.exists(df_cov_v3_path):
    ...
    if not os.path.exists(df_cov_v4_path):
