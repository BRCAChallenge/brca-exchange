...
class RemoveProblemVariant(DefaultPipelineTask):
    ...
    def run(self):
        ...
        cmd = 'grep -v "chr13:g.32398769:A>G" built_with_mupit.tsv | grep -v "chr13:g.32398768:T>G"| grep -v "chr13:g.32398770:A>G" |awk \'BEGIN {OFS=FS="\t"} { sub(/\.4$/, ".3", $357); print}\''
...
class PostProcessPriors(DefaultPipelineTask):
    ...
    def run(self):
        ...
        cmd = 'cat built_with_priors.tsv | awk \' BEGIN {FS=OFS="\t"} { sub(/\.3$/, ".4", $357); print}\''
        pipeline_utils.run_process(cmd,
