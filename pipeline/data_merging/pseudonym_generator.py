...
def _query_server(hgvs_ids: Iterable[str], url: str, max_chunk_size: int = 50000):
    ...
    grch37_alleles = [_extract_grch37_allele(r) for r in resp_all]
    return ca_ids, grch37_alleles
...
def genomic_hgvs(chr, pos, ref, alt):
    this_variant = VCFVariant(chr, pos, ref, alt, normalizer)
    tmp_genomic_hgvs_38 \
        = this_variant.to_hgvs_obj(hgvs_proc.contig_maps[HgvsWrapper.GRCh38_Assem])
    genomic_hgvs_38 = normalizer.normalize(tmp_genomic_hgvs_38)
    genomic_hgvs_str = str(tmp_genomic_hgvs_38)
    return(genomic_hgvs_str)
...
def map_via_seqrepo(this_gene, genomic_hgvs_str, default_cdna, normalizer,
    ...
    try:
        ...
        hg37_start = int(str(tmp_genomic_hgvs_37.posedit.pos.start))
        hg37_end = int(str(tmp_genomic_hgvs_37.posedit.pos.end))
        protein = str(assembly_mapper_38.c_to_p(tmp_cdna_hgvs))
        if debug:
            print("Protein HGVS:", protein)
    return(pyhgvs_cdna, genomic_coordinate_37, hg37_start, hg37_end, protein)
...
def main():
    ...
    cdna_default_ac_dict = {r[config.SYMBOL_COL]: r[config.HGVS_CDNA_DEFAULT_AC]
                            for _, r in config_df.iterrows()}
    syn_ac_dict = {r[config.SYMBOL_COL]: r[config.SYNONYM_AC_COL].split(';')
                   for _, r in config_df.iterrows()}
    chrom_ac_dict = {r[config.SYMBOL_COL]: r[config.CHROM_COL]
                   for _, r in config_df.iterrows()}
    ...
    with open(args.input, 'r') as input_fp:
        ...
        new_fieldnames = [PYHGVS_GENOMIC_COORDINATE_38_COL, PYHGVS_CDNA_COL, PYHGVS_GENOMIC_COORDINATE_37_COL,
                          PYHGVS_HG37_START_COL, PYHGVS_HG37_END_COL, PYHGVS_PROTEIN_COL, CA_ID_COL]
        rows = list(reader)
        output_fieldnames = reader.fieldnames + new_fieldnames

        hgvs_values = [str(genomic_hgvs(row[CHR_COL], int(row[POS_COL]),
                                        row[REF_COL], row[ALT_COL],
                                        genomic_normalizer)) for row in rows]
        logging.info("Querying ClinGen Allele Registry for CA IDs")
        ca_ids, grch37_alleles = _query_server(hgvs_values, ALLELES_ENDPOINT)

        processed_rows = []
        for row, genomic_hgvs_38, ca_id, grch37_allele in zip(rows, hgvs_values, ca_ids, grch37_alleles):
            row[PYHGVS_GENOMIC_COORDINATE_38_COL] = genomic_hgvs_38
            if args.debug:
                print("working on variant",
                      row[PYHGVS_GENOMIC_COORDINATE_38_COL])
            (row[PYHGVS_CDNA_COL],row[PYHGVS_GENOMIC_COORDINATE_37_COL],
             row[PYHGVS_HG37_START_COL], row[PYHGVS_HG37_END_COL],
             row[PYHGVS_PROTEIN_COL]) \
            ...
            new_synonyms = get_synonyms(this_gene, tmp_genomic_hgvs_38, syn_ac_dict[this_gene], hdp, gn, am38)
            updated_synonyms = _merge_and_clean_synonyms(row, new_synonyms)
            row[SYNONYMS_COL] = updated_synonyms
            row[CA_ID_COL] = ca_id if ca_id is not None else ''
            row[PYHGVS_GENOMIC_COORDINATE_37_COL] = grch37_allele
