...
def write_reports_tsv(filename, columns, ready_files_dir, gene_config_df):
    reports_output = open(filename, "w")

    reports_files = [ready_files_dir + r for r in get_reports_files(ready_files_dir)]

    reports = aggregate_reports(reports_files, columns, gene_config_df)
...
def aggregate_reports(reports_files, columns, gene_config_df):
    ...
    for file in reports_files:
        file_reports = normalize_reports(file, columns, gene_config_df)
...
def normalize_reports(file, columns, gene_config_df):
    filename, file_extension = os.path.splitext(file)
    if file_extension == ".vcf":
        reports = normalize_vcf_reports(file, columns, filename, file_extension, gene_config_df)
...
def normalize_vcf_reports(file, columns, filename, file_extension, gene_config_df):
    reports = []
    ...
    for record in reader:
        ...
        report = associate_chr_pos_ref_alt_with_item(record, len(columns), source, genome_coor, gene_config_df)
